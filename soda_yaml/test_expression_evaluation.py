...
def init_for_dynamic():
    init()
    import builtins
    builtins.SODA_FILES_IN_ROOT = ['tests_data/data/scope_1/scope_2_1/scope_3-b_scope_4_a',
                                   'tests_data/data/scope_1/scope_2_a/scope_3-b_scope_4_a',
                                   'tests_data/data/scope_1/scope_2_b/scope_3-b_scope_4_a',]
...
def test_evaluate_static_expr(init):
    to_test = evaluate_yaml_expression("${SCOPE_2_DIGIT} is a static expression")
    assert to_test == "scope_2_\\d/ is a static expression"


def test_evaluate_nested_static_expr(init):
    to_test = evaluate_yaml_expression("${SCOPE_2} is a nested static expression")
    assert to_test == "(scope_2_\\d/|scope_2_[a-zA-Z]/) is a nested static expression"


def test_evaluate_dynamic_expr(init_for_dynamic):
    to_test = evaluate_yaml_expression("?{SCOPE_2_LETTER} is a dynamic expression", 
...
def test_evaluate_static_expr_nested_in_dynamic(init_for_dynamic):
    to_test = evaluate_yaml_expression("?{SCOPE_2} is a dynamic expression "
                                       ...
                                       current_expr=init_for_dynamic)
    assert to_test == ("scope_2_b/ is a dynamic expression with a static one nested in it")

def test_evaluate_dynamic_expr_nested_in_static_expr(init_for_dynamic):
    to_test = evaluate_yaml_expression("${SCOPE2_DYNAMIC} is a static expression "
                                       "with a dynamic one nested in it", 
                                       current_expr=init_for_dynamic)
    assert to_test == ("scope_2_b/ is a static expression with a dynamic one nested in it")
